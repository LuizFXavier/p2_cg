...
MainWindow::beginInitialize()
{
  ...
  p2App = std::make_unique<P2Window>(width(), height());
  p2App->initialize();
...
MainWindow::gui()
{
  ...
  if (ImGui::BeginMainMenuBar()) {
    ...
    if (ImGui::BeginMenu("Application Mode")) {
      ...
      if (ImGui::MenuItem("Demo", nullptr, appMode == AppMode::DEMO)) {
      ...
      if (ImGui::MenuItem("P2", nullptr, appMode == AppMode::P2)) {

        appMode = AppMode::P2;

        _viewMode = ViewMode::Renderer;

        p2App->resize(width(), height());
